...
This site uses giscus (GitHub Discussions) for comments. It works entirely client-side and is compatible with GitHub Pages.

1. Enable Discussions on the repository used for this site (e.g., `rsinghvi/articles`). In GitHub, go to Settings → Discussions → Enable.
2. Visit https://giscus.app and configure:
   - Repository: `rsinghvi/articles`
   - Category: create/select one (e.g., "General")
   - Mapping: `pathname`
   - Theme: `light` (matches site)
3. Copy the `repo_id` and `category_id` values generated by giscus.
4. Update `_config.yml` under `giscus:` with:
   - `repo`: `rsinghvi/articles`
   - `repo_id`: the ID from giscus
   - `category`: your chosen category name
   - `category_id`: the ID from giscus
   - Optional: adjust `mapping`, `theme`, `lang`.
5. Commit and push. Comments appear below each post.

Moderation tips:
- Use Discussion settings in GitHub to manage and moderate threads.
- Reactions are enabled by default; disable via `_config.yml` if needed.
